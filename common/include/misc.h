...
static void swap(int* ptrA, int* ptrB){
    int tmp=*ptrA;
    *ptrA=*ptrB;
    *ptrB=tmp;
}

static int negative(const int i)
{
    return ((i<0)?i:((-i)-1));
}

static int absolute(const int i)
{
    return ((i>=0)?i:((-1)*i-1));
}
...
static void gnomeSortByKeyInc(int *arr, const int *key, const int from, const int to) {
    int i=from+1;
    int j=i+1;
    while(i<=to){
...
static void gnomeSortByKeysInc(int *arr, const int *key1, const int *key2, const int from, const int to) {
    int i=from+1;
    int j=i+1;
    while(i<=to){
        if( (key1[arr[i-1]] > key1[arr[i]]) || ((key1[arr[i-1]] == key1[arr[i]]) && (key2[arr[i-1]] > key2[arr[i]])) ){
            swap(&arr[i-1],&arr[i]);
            if(--i)continue;
        }
        i = j++;
...
static bool isSmaller(const int *a,const int *b, int N)
{
    for(int i=0;i<N;i++)
    {
